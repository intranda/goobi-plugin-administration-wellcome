...
//                Helper.deleteDir(new File(process.getProcessDataDirectory()));
	...
	public String deleteFromPlayer() {
		...
		Path destinationFolder = Paths.get(destination, first, second, third, fourth);
//		File destinationFolder = new File(
//				destination + first + File.separator + second + File.separator + third + File.separator + fourth);
		Path fileToDelete = destinationFolder.resolve(bnumber + ".xml");
//		File fileToDelete = new File(destinationFolder, bnumber + ".xml");
		if (!StorageProvider.getInstance().isFileExists(fileToDelete)) {
//		if (!fileToDelete.exists()) {
			...
			try {
				Document doc = new SAXBuilder().build(StorageProvider.getInstance().newInputStream(fileToDelete));
//				Document doc = new SAXBuilder().build(fileToDelete);
			...
			try {
				StorageProvider.getInstance().deleteFile(fileToDelete);
//				FileUtils.forceDelete(fileToDelete);
		...
		if (mmo) {
			...
			Path anchorFile = destinationFolder.resolve(identifier+".xml");
//			File anchorFile = new File(destinationFolder, identifier + ".xml");

			try {
				Document doc = new SAXBuilder().build(StorageProvider.getInstance().newInputStream(anchorFile));
...
//					FileUtils.forceDelete(anchorFile);
					...
					OutputStream output =StorageProvider.getInstance().newOutputStream(anchorFile);
//					FileOutputStream output = new FileOutputStream(anchorFile);
		...
		// delete alto as well
		Path altoFolder = destinationFolder.resolve(bnumber + "_alto");
		if (StorageProvider.getInstance().isDirectory(altoFolder)) {
//		if (altoFolder.exists()) {
//			try {
				StorageProvider.getInstance().deleteDir(altoFolder);
